...
**Last updated:** 2026-07-19 (flip #16; N56 Gemini-retention half descoped as accepted risk — confirmed free-tier Gemini API (GOOGLE_API_KEY, no billing), Google may train on prompts/audio; not fixed, redundant against Phase 6 RAG replacement (Chirag). §11 Gemini fork reclassified VERIFY→ACCEPTED RISK. W10 fully descoped: W10a closed (flip #15), W10b descoped (this flip). Docs-only commit, no code/branch/PR; baseline unchanged 51/9; §2 Current main sha unchanged at `4d434c8` — no feature merge since flip #15)
...
- **Default branch:** `main`
- **Current main sha:** `4d434c8` (#48 — W10a E2E-badge rescope squash-merge; last feature merge)
- **Last merge:** #48 W10a — rescoped false "End-to-End Encrypted Workspace" badge to "Encrypted in transit & at rest" (SettingsClient.tsx:235), closes N56 UI-badge portion (`4d434c8`)
...
- **WS5 branch:** `feat/ws5-kanban-dragdrop` MERGED via #45 (squashed to `c35f0d3`).
...
- ~~**/tasks Orchestration sidebar lacks completed-state visual treatment**~~ ✅ closed @ `c35f0d3` (#45, WS5). Dim + strikethrough implemented on completed tasks in the compact sidebar row list, `TasksClient.tsx`.
...
- **Open verify (WS5, #45)**: `updateProfile` (`settingsActions.ts`) writes `User.name` but never calls NextAuth `session.update()`. `/tasks` bucketing reads `session.user.name` from the login-time JWT. Code-level read only, never runtime-observed. Repro: rename to non-empty value in Settings, check whether a My Plate drop writes old or new name. Low severity — worst case is stale bucketing until re-login, not a correctness break.
...
- **N56 (P2)** — UI asserts "End-to-End Encrypted Workspace" — false: server-side AI processing; voice audio is written to server tmpdir and uploaded to Gemini File API. SOT §11 flags the pitch-deck claim; the in-app badge was uncatalogued. For a legal-confidentiality product this is a liability, not copy polish. Evidence: SettingsClient.tsx:235; supporting: transcribe/route.ts:43–51. Fix direction: Rescope to "Encrypted in transit & at rest"; pair with Gemini-retention verification. Resolution: W10. **W10a ✅ @ `4d434c8` (#48)**: UI-badge portion closed — badge text rescoped to "Encrypted in transit & at rest" at SettingsClient.tsx:235. Gemini-retention portion (data-training/retention verification) remains **OPEN** — tracked as W10b. **W10b DESCOPED (accepted risk)** — Gemini API confirmed free-tier (GOOGLE_API_KEY, no billing enabled); Google may train on prompts/audio under current terms. Not fixed — Gemini API is being replaced entirely by Phase 6 RAG (Chirag), engineering work here redundant against that migration. Risk accepted by plan-author. Moot once Phase 6 lands (Gemini removed, not re-tiered).
...
- **N59 (P2) ✅ closed at W9 PR3 (#47, 1738326)** — deleteCase cascade is (a) non-atomic — 5 sequential deleteMany on the per-op scoped client, mid-flight failure orphans children; (b) misses Document — Document.caseId FK is SET NULL, so on case delete Documents survive with caseId nulled (silent orphaning, not a delete-block as originally recorded — recon §5 correction). Data-hygiene debt: orphaned Documents pollute the inbox as "unsorted" indefinitely. Latent until Document upload ships, then immediate. Evidence: caseActions.ts:389–417; schema relation schema.prisma:65, 139. Fix direction: Wrap in withServerUserContext, add document.deleteMany; superseded by onDelete: Cascade. Resolution: W9. Closed: wrapped in withServerUserContext, added document.deleteMany, +1 RLS assertion (verify-phase32-rls Check 9). Belt-and-suspenders posture preserved on Note/CalendarEvent app-side deletes despite PR1 DB Cascade — cleanup finding logged as N87.
...
- **N71 (P1)** — ✅ closed @ `0179f38` (#39, W1, Decision A). PR8 WS1 uplifted `updateTaskAssignee` (Zod + withServerUserContext + Result envelope), did NOT delete it — original SOT N53 handoff wording was wrong. W1 Decision A: keep PR8's uplifted version (0 callers on both sides, inert but RLS-safe; preserves optionality for potential WS5 assignee-drag use). Main's bare-Prisma variant (last bare-Prisma fn in module, N38 trap class) overwritten. `getTasks` still deleted per WS1 as originally scoped. SOT N53 wording corrected via this flip. Codicil (WS5, #45, c35f0d3): closure text's "or falls through to kept updateTaskAssignee" is inaccurate. Verified at WS5 recon (2026-07-17) and reconfirmed at WS5 execution (#45): updateCaseTask's OR-clause alone handles independent-task assignee writes; no fallthrough path exists in code. updateTaskAssignee remains dormant, 0 callers.
...
**W9 PR3 — deleteCase atomicity (2026-07-19)**
- **N87 (P3)** — Redundant Note/CalendarEvent app-side deleteMany in deleteCase post-W9 PR1 Cascade uplift. Both DB-level ON DELETE CASCADE (PR1) and app-side tx.note.deleteMany / tx.calendarEvent.deleteMany (PR3) fire on Case delete — the app-side calls are now redundant with the DB cascade but retained by design under W9 PR3 Option A (belt-and-suspenders posture: defensive against future schema drift, mirrors Payment→Restrict audit-boundary logic from PR1). Evidence: caseActions.ts deleteCase body, PR1 migration 20260717122932 comments. Fix direction: drop tx.note.deleteMany + tx.calendarEvent.deleteMany, rely on FK Cascade. Resolution: post-W9, if schema is confirmed stable. Low priority — code is correct today, this is architectural cleanup only.
...
| Gemini data retention vs no-retention tier | **ACCEPTED RISK** — confirmed free-tier (Google may train on prompts/audio under current terms); not fixed, redundant against Phase 6 RAG replacement (Chirag). Revisit only if Phase 6 slips materially. |
| "E2E encrypted, legal-grade" marketing claim | **RESOLVED @ W10a (PR #48, `4d434c8`)** — badge rescoped to "Encrypted in transit & at rest" (SettingsClient.tsx:235). |
...
| W9 | Schema-cleanup pass (PR1 ✅ / PR2 ✅ / PR3 ✅ / PR4a abandoned / PR4b dropped — W9 closed) | **PR1 ✅ @ `eaa71d1` (#44)** — Payment.status → Postgres enum PaymentStatus; FK-norm on Case children: Note→Cascade + CalendarEvent→Cascade (operational cleanup, deliberate uplift), Task→SetNull (N86 fix, sole genuine drift), Payment→Restrict retained (financial audit boundary); 7 opportunistic N61 indexes (Note ×2, Task ×2, Payment ×2, CalendarEvent ×1); TS narrowing on `financeActions.ts` to `z.nativeEnum(PaymentStatus)`. Closes N85 (via process fix), N86. Zero verify delta. **PR2 ✅ @ `702407f` (#46)** — Case.nextHearingDate column drop, live CalendarEvent groupBy migration via getNextHearingDatesByCase helper, calendar-sync.ts + backfill + stale-past filter deletion, CalendarEvent(caseId, hearingDate) composite index add. Zero verify delta. **PR3 ✅ @ `1738326` (#47)** — deleteCase wrapped in withServerUserContext (multi-write atomicity), document.deleteMany added inside tx (closes N59 Document orphan gap), Check 9 added to verify-phase32-rls.ts (cross-tenant Document isolation, structural-only per N40). Baseline 50/9 → 51/9. Belt-and-suspenders posture on Note/CalendarEvent preserved per Option A; deferred finding logged as N87. **PR4a abandoned** (Float→Int paise migration, reverted pre-push); **PR4b dropped** — Float64 exact at Lawdger's scale. W9 closed. | After W5+W8 |
| W10 | Trust-claims reconciliation (N56) (W10a ✅ / W10b descoped) | **W10a ✅ @ `4d434c8` (#48)** — Badge rescoped from "End-to-End Encrypted Workspace" to "Encrypted in transit & at rest" (SettingsClient.tsx:235). Closes N56 UI-badge portion. **W10b descoped (accepted risk, moot at Phase 6)** — Gemini API confirmed free-tier, may train on data; not fixed, redundant against Phase 6 RAG replacement (Chirag). | Parallel |
| W11 | Phase 9 — production cutover (PR1–PR5 scoped, not started) | **PR1** — DIRECT_URL fix: re-point to genuine session-pooler endpoint (port 5432, session mode), off Supabase IPv4 paid add-on; retires MCP-detour workaround (L83-85, L334). **PR2** — env validator widen: `src/env.ts` Zod schema extended to cover `GOOGLE_API_KEY` + `LLM_*` (N37), create `.env.local.example` (currently missing), resolve `GITHUB_TOKEN` dead-var question (present in `.env.local`, absent from `.env.example`, unreferenced in `src/`). **PR3** — rate-limiting via Upstash Redis (chosen over in-memory — doesn't survive Vercel serverless scale-out), wired on `/api/voice/transcribe` + `/api/chat` (N22). **PR4** — `next-auth` beta→stable upgrade (N32) — ISOLATED, own CP-sequence/session/smoke pass; high-risk (breaking-change potential in session-callback shape / JWT encoding, touches `auth.ts`/`auth.config.ts`/`proxy.ts` together). **PR5** — Vercel `DATABASE_URL` prod swap (`postgres`/BYPASSRLS → `lawdger_app`/FORCE RLS) + prod smoke, mirrors local 3.0.1d cutover (L139-140) — LAST, gated on PR1–4. Sequencing: PR1/PR2/PR3 may interleave or run in parallel; PR4 isolated; PR5 last. | After W9 |
...
**Critical path:** ~~W1~~ → ~~W3~~ → ~~W5~~ → ~~W4~~ → ~~W8~~ → ~~**W9**~~ (PR1 ✅ / PR2 ✅ / PR3 ✅ / PR4a abandoned / PR4b dropped) → W11.
...
- Fable ledger: 0 credits remaining. Spent on WS5 (#45), 2026-07-17 — the reserved credit held since flip #9 (design review for the Kanban drag-drop cross-bucket-vs-intra-bucket-reorder fork). Prior handoff had projected W5.1 taxonomy design as consuming the last credit; actual W5.1 execution used Opus 4.7 architectural stress-test instead (no Fable model invoked), leaving the credit intact until WS5 drew on it as originally reserved.
