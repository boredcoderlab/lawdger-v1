...
| W4 | Silent-failure UI sweep (N50/N51 client half) | Every mutation caller checks Result | Unblocked — next after W5.1 |
...
| W5.1 | Error taxonomy — WS-G deferred from W5 | Discriminated Result variant OR error code enum for ~35 return sites; picks alignment with dispatch.ts LLM-facing surface (currently forwards `result.error` verbatim). Recon (W5 CP-3) surfaced 3 options; execution deferred until W4 client sweep or W12 notifications creates a real consumer. | Attach on next consumer emergence |
...
**Critical path:** ~~W1~~ → ~~W3~~ → ~~W5~~ → **W4** → W8 → W9 → W11.
