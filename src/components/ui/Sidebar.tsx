...
export default function Sidebar() {
  ...
  return (
    <aside className="w-24 h-[calc(100vh-3rem)] bg-lawdger-espresso rounded-[2.5rem] flex flex-col items-center py-6 relative z-50 shadow-[0_20px_60px_rgba(44,36,32,0.12)] border border-white/5 my-6 ml-6 shrink-0">
      ...
      {/* Brand Anchor */}
      <div className="w-12 h-12 bg-lawdger-base rounded-[14px] flex items-center justify-center shadow-[0_4px_12px_rgba(0,0,0,0.4),inset_0_2px_4px_rgba(255,255,255,0.9)] mb-10 cursor-pointer hover:scale-105 transition-transform duration-300 relative z-20">
                  ...
                  >
                      <item.icon 
                          ...
                          className={`transition-all duration-500 ${
                              isActive ? "text-lawdger-espresso scale-110 drop-shadow-sm" : "text-[#8A8078] group-hover:text-[#D4C9C0] group-hover:scale-110" 
                          }`} 
                      />
                      {item.alert && (
                          <span className={`absolute top-3 right-5 w-2.5 h-2.5 bg-destructive rounded-full border-[2.5px] transition-colors duration-500 ${isActive ? "border-lawdger-base" : "border-lawdger-espresso"}`}></span>
                      )}
                  </div>
              );
          })}
